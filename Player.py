...
class Player(Ship):
    ...
    def move(self):
        ...
        if (keys[pygame.K_UP] or keys[pygame.K_w]) and self.y > 0:
            self.y -= self.y_speed
        if (keys[pygame.K_DOWN] or keys[pygame.K_s]) and self.y < HEIGHT - self.ship_img.get_height():
            self.y += self.y_speed
    ...
    def create_bullet(self):
        if (len(self.bullet) < self.max_amount_bullets) and self.creation_cooldown_counter == 0:
    ...
    def fire(self, window):
        ...
        if (key[pygame.K_SPACE]) and (len(self.bullet) > 0) and (self.bullet_cooldown_counter == 0):
            self.bullet[-1].x = self.x + (self.ship_img.get_width() - self.bullet_img.get_width()) // 2
            self.bullet[-1].y = self.y + 10
            self.fired_bullets.append(self.bullet.pop())
            self.bullet_cooldown_counter = 1
            self.creation_cooldown_counter = 1
    ...
    def hit(self, enemy):
        for i in range(len(self.fired_bullets)):
            ...
            return self.fired_bullets[i].colision(enemy)